...
export default function AnalizadorPromos() {
      ...
      try {
        ...
        for (let i = filaCabeceras + 1; i < filasBrutas.length; i++) {
          ...
          if (valNum === 100 || valNum === 1) {
            ...
            const fecha = colFecha !== -1 ? String(fila[colFecha] || '').trim() : '';
        ...
        for (let i = filaCabeceras + 1; i < filasBrutas.length; i++) {
          ...
          if (identificadores.has(`${cliente}_${fecha}_${docum}_${codigo}`)) {
            const filaLimpia = [...fila];
            if (colFecha !== -1 && typeof filaLimpia[colFecha] === 'number') {
              const diasDesde1900 = filaLimpia[colFecha];
              const fechaObj = new Date(Math.round((diasDesde1900 - 25569) * 86400 * 1000));
              filaLimpia[colFecha] = fechaObj.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
            }
            filasFiltradas.push(filaLimpia);
          }
        }
  ...
  const descargarPDF = () => {
    ...
    doc.setFontSize(12); doc.setTextColor(30, 41, 59); doc.text('TOTAL BOTELLAS SIN CARGO:', 20, 56);
    doc.setFontSize(24); doc.setTextColor(37, 99, 235); doc.text(`${totalBotellas}`, 150, 62, { align: 'right' });
    doc.setFontSize(14); doc.setTextColor(30, 41, 59); doc.text('Desglose de productos:', 14, 85);

    autoTable(doc, {
      startY: 90, head: [['Descripción del Producto', 'Cantidad (Unidades)']], body: resumenProductos.map(p => [p.descripcion, p.cantidad]),
        ...
        <div className="bg-white p-8 rounded-xl border-2 border-dashed border-blue-200 text-center flex flex-col justify-center min-h-[300px]">
          {procesando ? <Loader2 size={40} className="animate-spin mx-auto text-blue-600" /> : (
            <div className="flex flex-col items-center">
              <FileDown size={40} className="text-blue-600 mb-4" /><h3 className="text-xl font-bold mb-2">1. Sube tu Excel</h3>
      ...
      {datosExtraidos.length > 0 && (
        <div className="bg-slate-900 rounded-xl p-6 shadow-xl text-white animate-fade-in-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <select value={bodegaSel} onChange={(e) => {setBodegaSel(e.target.value); setAcuerdoSel(''); setReclamacionSel('');}} className="p-2 rounded bg-slate-800 border border-slate-700 text-sm uppercase"><option value="">-- Bodega --</option>{bodegasUnicas.map(b => <option key={b} value={b}>{b}</option>)}</select>
            <select value={acuerdoSel} onChange={(e) => {setAcuerdoSel(e.target.value); setReclamacionSel('');}} disabled={!bodegaSel} className="p-2 rounded bg-slate-800 border border-slate-700 text-sm disabled:opacity-30"><option value="">-- Cliente/Acuerdo --</option>{acuerdosDeBodega.map(a => <option key={a.id} value={a.id}>{a.cliente}</option>)}</select>
            <select value={reclamacionSel} onChange={(e) => setReclamacionSel(e.target.value)} disabled={!acuerdoSel} className="p-2 rounded bg-slate-800 border border-slate-700 text-sm disabled:opacity-30"><option value="">-- Periodo --</option>{reclamacionesDisponibles.map((r:any) => <option key={r.id} value={r.id}>{r.periodo}</option>)}</select>
          </div>
          <button onClick={vincularYGuardar} disabled={!reclamacionSel || guardando} className="w-full bg-blue-600 hover:bg-blue-500 py-3 rounded-lg font-bold flex justify-center items-center gap-2 transition disabled:bg-slate-700">{guardando ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />}Vincular y Guardar</button>
        </div>
      )}
